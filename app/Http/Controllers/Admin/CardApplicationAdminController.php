...
class CardApplicationAdminController extends Controller
{
    ...
    public function approve($application, Request $request)
    {
        ...
        $orgTheme = [
            'bg' => $appTheme['bg'] ?? $bg,
            'surface' => $appTheme['surface'] ?? '#0f172a',
            'ink' => $appTheme['ink'] ?? '#ffffff',
            'muted' => $appTheme['muted'] ?? '#94a3b8',
            'line' => $appTheme['line'] ?? '#1e293b',
            'accent' => $appTheme['accent'] ?? $accent,
            'accent_dark' => $appTheme['accent_dark'] ?? '#9e7d3b',
            'accent_soft' => $appTheme['accent_soft'] ?? Organization::hexToRgba($accent, 0.15),
            'dark' => $appTheme['dark'] ?? '#030712',
