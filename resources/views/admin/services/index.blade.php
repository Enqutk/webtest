...
@section('content')
@php
    $liveServicesUrl = route('card.services.index', ['slug' => $currentOrg->slug]);
@endphp

<div class="mb-5 bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
    <div class="flex items-center justify-between p-4 border-b border-slate-100">
        <div class="space-y-0.5">
            <h3 class="text-sm font-bold text-slate-900">Live Services Preview</h3>
            <p class="text-xs text-slate-500">This iframe shows the currently saved services page.</p>
        </div>
        <a href="{{ $liveServicesUrl }}" target="_blank" rel="noopener"
...
<div class="space-y-6" x-data="{
    ...
    create() {
        ...
        this.shortDesc = '';
        this.quote = '';
        this.order = {{ ($services->max('order') ?? 0) + 1 }};
        this.status = 'active';
        this.openModal = true;
    },
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm">
        <div>
            <h2 class="text-base font-bold text-slate-900">Services Catalog</h2>
            <p class="text-xs text-slate-500 mt-0.5">Manage service offerings, capability statements, and showcase details for {{ $currentOrg->title }}.</p>
        </div>
        <button type="button" @click="create()" class="inline-flex items-center gap-2 px-4 py-2 bg-brand-600 hover:bg-brand-500 text-white text-xs font-bold rounded-xl shadow-lg shadow-brand-600/30 transition shrink-0">
            ...
            <span>Add New Service (Modal)</span>
        </button>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($services as $srv)
            @php
                $img = $srv->getFirstMediaUrl('service-images');
            @endphp
            <div class="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-sm hover:shadow-md transition flex flex-col justify-between">
                <div>
                    <div class="flex items-start justify-between gap-3 mb-3">
                        <span class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 font-bold text-xs flex items-center justify-center shrink-0">
                            {{ $srv->order }}
                        </span>
                    ...
                    @if($img)
                        <div class="h-32 rounded-xl bg-slate-100 overflow-hidden mb-3">
                            <img src="{{ $img }}" alt="{{ $srv->title }}" class="w-full h-full object-cover">
                        </div>
                    @endif

                    <h3 class="text-sm font-bold text-slate-900 mb-1">{{ $srv->title }}</h3>
                    <p class="text-xs text-slate-500 line-clamp-3 mb-4">{{ $srv->short_description }}</p>
                </div>

                <div class="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
                    <button type="button" @click="edit({{ json_encode($srv) }})" class="px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 text-xs font-bold rounded-lg transition">
                        Edit
                    </button>
        ...
        <div class="min-h-full px-4 py-8 flex items-center justify-center">
            ...
            <div class="relative bg-white rounded-2xl border border-slate-200 shadow-2xl max-w-2xl w-full p-6 lg:p-8 space-y-6 z-10">
                <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                    <h3 class="text-base font-bold text-slate-900" x-text="editingId ? 'Edit Service' : 'Add New Service'"></h3>
                    <button @click="openModal = false" class="text-slate-400 hover:text-slate-700"><i class="bi bi-x-lg"></i></button>
                </div>

                <form :action="editingId ? ('/admin/services/' + editingId) : '{{ route('admin.services.store') }}'" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    ...
                    <div class="space-y-1.5">
                        <label class="block text-xs font-bold text-slate-700">Service Title <span class="text-rose-500">*</span></label>
                        <input type="text" name="title" x-model="title" required placeholder="e.g. Climate-Smart Irrigation Schemes" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:bg-white transition">
                    </div>

                    <div class="space-y-1.5">
                        <label class="block text-xs font-bold text-slate-700">Short Summary</label>
                        <textarea name="short_description" x-model="shortDesc" rows="2" placeholder="Brief summary displayed on homepage cards" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:bg-white transition"></textarea>
                    </div>

                    <div class="space-y-1.5">
                        <label class="block text-xs font-bold text-slate-700">Client Quote / Highlight</label>
                        <input type="text" name="quote" x-model="quote" placeholder="e.g. Robust gravity-fed irrigation with community maintenance handover" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:bg-white transition">
                    </div>
