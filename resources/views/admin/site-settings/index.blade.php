...
@section('page-form')
    ...
    <div class="site-settings-panel space-y-4" x-data="{
            activeTab: 'header',
        ...
        >
            ...
            <div x-show="activeTab === 'header'" x-cloak
                class="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-sm space-y-5">
                <div class="border-b border-slate-100 pb-3">
                    <h3 class="text-sm font-bold text-slate-900">Header & brand bar</h3>
                    <p class="text-xs text-slate-500 mt-0.5">Logo, company name, tagline, and the Get in touch button shown
                ...
                <div id="tagline" class="space-y-1.5 scroll-mt-4">
                    ...
                    <input type="text" name="tagline" value="{{ $currentOrg->tagline }}"
                ...
                <div id="header-cta" class="pt-2 border-t border-slate-100 space-y-3 scroll-mt-4">
                    ...
                    <div class="grid grid-cols-1 gap-3">
                        <input type="text" name="theme[header_cta_text]"
                            value="{{ $theme['header_cta_text'] ?? 'Get in touch' }}" placeholder="Button text"
                            data-preview-bind="site-header-cta:header-cta-text"
                            class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs">
                        <input type="text" name="theme[header_cta_url]" value="{{ $theme['header_cta_url'] ?? '/contact' }}"
                            placeholder="/contact"
                    ...
                    <div class="space-y-2" x-data="{
                        ...
                        <template x-for="(email, i) in emails" :key="'email-'+i">
                            <div class="flex gap-2">
                                <input type="email" :name="'contact_emails['+i+']'" x-model="emails[i]"
                                    placeholder="Email address"
                                    class="flex-1 px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs">
                                <button type="button" @click="emails.splice(i, 1)" x-show="emails.length > 1"
                                    class="px-2 text-rose-500"><i class="bi bi-x-lg"></i></button>
                            </div>
                        </template>
                        <button type="button" @click="addEmail()" class="text-[11px] font-bold text-brand-700">+ Add
                            email</button>

                        <template x-for="(phone, i) in phones" :key="'phone-'+i">
                            <div class="flex gap-2 mt-2">
                                <input type="text" :name="'contact_phones['+i+']'" x-model="phones[i]"
                        ...
                        <button type="button" @click="addPhone()" class="text-[11px] font-bold text-brand-700">+ Add
                            phone</button>

                        <textarea name="address" rows="2" placeholder="Street address"
                            data-preview-bind="site-contact:address"
                            class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs mt-2">{{ $currentOrg->address }}</textarea>
                        <input type="text" name="po_box" value="{{ $currentOrg->po_box }}" placeholder="P.O. Box"
                            class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs">
                    </div>
                </div>
            ...
            {{-- Colors --}}
            <div x-show="activeTab === 'colors'" x-cloak id="branding"
                class="bg-white rounded-2xl border border-slate-200/80 p-5 lg:p-6 shadow-sm space-y-6 scroll-mt-4">
                <div class="border-b border-slate-100 pb-3">
                    <h3 class="text-sm font-bold text-slate-900">Background, colors & styling</h3>
                    <p class="text-xs text-slate-500 mt-0.5">Pick a 1-click theme preset or customize each color with
                        swatches and hex values.</p>
                </div>
                ...
                <p class="text-[11px] text-slate-500 pt-2 border-t border-slate-100">
                    ...
                    <a href="{{ route('admin.organizations.edit', $currentOrg) }}"
