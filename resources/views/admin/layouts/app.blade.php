...
<body class="h-full font-sans antialiased text-slate-800 bg-slate-50 flex" x-data="{
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto p-4 lg:p-8 custom-scrollbar">
            @if(!request()->routeIs('admin.dashboard', 'admin.organizations.edit', 'admin.home-sections.*', 'admin.site-pages.*', 'admin.services.*', 'admin.portfolio.*', 'admin.site-settings.*', 'admin.team.*'))
                @php
                    $activeOrg = $currentOrg ?? \App\Models\Organization::resolveCurrent();
                @endphp
