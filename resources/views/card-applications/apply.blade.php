@php
    ...
    $inviteDraftKey = $isInvite ? 'kimem-invite-' . $invitation->token : '';
    $kimemSupportPhone = config('seo.platform.support_phone', '+251911223344');
    $kimemSupportWhatsapp = preg_replace('/[^0-9]/', '', config('seo.platform.support_whatsapp', $kimemSupportPhone));
@endphp
<!DOCTYPE html>
...
<body class="min-h-full antialiased selection:bg-gold-500 selection:text-slate-950 pb-28 lg:pb-12 @if($isInvite) invite-flow bg-white text-slate-900 @else bg-[#070b14] text-slate-100 @endif"
      x-data='{
          ...
          saveDraft() {
              ...
              const payload = {
                  ...
                  bio: this.bio,
                  card_edition: this.card_edition,
                  ...
                  proj1_title: this.proj1_title,
                  proj1_tag: this.proj1_tag,
                  proj1_desc: this.proj1_desc,
                  proj1_url: this.proj1_url,
                  proj2_title: this.proj2_title,
                  proj2_tag: this.proj2_tag,
                  proj2_desc: this.proj2_desc,
                  proj2_url: this.proj2_url,
                  proj3_title: this.proj3_title,
                  proj3_tag: this.proj3_tag,
                  proj3_desc: this.proj3_desc,
                  proj3_url: this.proj3_url,
              };
          ...
          loadDraft() {
              ...
              try {
                  ...
                  Object.keys(data).forEach((key) => {
                      if (key in this && data[key] !== undefined && data[key] !== null) {
                          this[key] = data[key];
                      }
                  });
    ...
    <header class="sticky top-0 z-40 border-b backdrop-blur-xl @if($isInvite) invite-header border-slate-200 bg-white/95 @else border-slate-800/80 bg-slate-950/90 @endif">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-14 sm:h-16 flex items-center justify-between">
            <div class="flex items-center gap-2.5">
                ...
                <div>
                    <span class="font-bold text-sm tracking-wide font-cinzel @if($isInvite) text-slate-900 @else text-white @endif">KIMEM</span>
                    <span class="text-[9px] block font-mono text-gold-500 font-medium uppercase tracking-widest -mt-1">
            ...
            <div class="flex items-center gap-1.5 sm:gap-2">
                @if($isInvite)
                    <button type="button" @click="saveDraft()"
                            class="px-2.5 sm:px-3 py-1.5 rounded-xl border border-slate-200 bg-white text-slate-700 font-bold text-[10px] sm:text-xs flex items-center gap-1 hover:border-gold-400 transition">
                        <i class="bi" :class="draftSaved ? 'bi-check-lg text-emerald-600' : 'bi-bookmark'"></i>
                        <span class="hidden sm:inline" x-text="draftSaved ? 'Saved' : 'Save'"></span>
                    </button>
                    <a href="tel:{{ preg_replace('/[^0-9+]/', '', $kimemSupportPhone) }}"
                       class="px-2.5 sm:px-3 py-1.5 rounded-xl border border-emerald-200 bg-emerald-50 text-emerald-800 font-bold text-[10px] sm:text-xs flex items-center gap-1 hover:bg-emerald-100 transition">
    ...
    <div class="fixed bottom-0 inset-x-0 z-30 lg:hidden mobile-bottom-bar invite-bottom-bar p-3 border-t backdrop-blur-xl flex items-center gap-2">
        @if($isInvite)
        <button type="button" @click="saveDraft()" class="py-3 px-3 rounded-xl border text-xs font-bold flex items-center justify-center min-w-[44px] bg-white border-slate-200 text-slate-600">
            <i class="bi" :class="draftSaved ? 'bi-check-lg text-emerald-600' : 'bi-bookmark'"></i>
        </button>
        <a href="tel:{{ preg_replace('/[^0-9+]/', '', $kimemSupportPhone) }}" class="py-3 px-3 rounded-xl border text-xs font-bold flex items-center justify-center min-w-[44px] bg-emerald-50 border-emerald-200 text-emerald-700">
            <i class="bi bi-telephone-fill"></i>
        </a>
        @else
        <button type="button" @click="mobilePreviewExpanded = !mobilePreviewExpanded; window.scrollTo({ top: 0, behavior: 'smooth' })" class="py-3 px-3 rounded-xl border text-xs font-bold flex items-center justify-center gap-1.5 min-w-[44px] bg-slate-900 border-slate-800 text-white">
            <i class="bi bi-eye text-gold-400"></i>
        </button>
        @endif
        <button type="button" @click="activeStep > 1 ? activeStep-- : null" :disabled="activeStep === 1" class="py-3 px-4 rounded-xl border text-xs font-bold disabled:opacity-40 @if($isInvite) bg-white border-slate-200 text-slate-600 @else bg-slate-900 border-slate-800 text-slate-300 @endif">
            Back
        </button>
        <button type="button" @click="activeStep < 4 ? (saveDraft(), activeStep++) : document.querySelector('form').requestSubmit()" :disabled="submitting" class="flex-1 py-3 px-3 rounded-xl bg-gold-500 text-slate-950 font-extrabold text-xs flex items-center justify-center gap-1 shadow-md shadow-gold-500/20 disabled:opacity-60 min-h-[44px]">
            <span x-text="submitting ? 'Sending…' : (activeStep === 4 ? (isInvite ? 'Submit My Card' : 'Submit Request') : 'Continue →')"></span>
        </button>
    </div>
