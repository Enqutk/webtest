...
@php
    ...
    $aboutHref = $linkUrl ?: ($data['aboutUrl'] ?? route('about'));
    $siteName = $data['siteName'] ?? config('app.name');
    $initials = collect(preg_split('/\s+/', trim((string) $siteName)))
        ->filter()
        ->take(2)
...
<section class="hz-section hz-about {{ $isMe ? 'hz-about-me' : '' }}" id="{{ $sectionId }}"
